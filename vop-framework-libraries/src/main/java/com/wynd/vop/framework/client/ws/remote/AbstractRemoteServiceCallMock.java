...
public abstract class AbstractRemoteServiceCallMock implements RemoteServiceCall {
	private static final VopLogger LOGGER = VopLoggerFactory.getLogger(AbstractRemoteServiceCallMock.class);
	...
	private ResourceSource readMockResponseByKey(final PartnerTransferObjectMarker request) {
		...
		try {
			...
			MessageKeys msgkey = MessageKeys.VOP_REMOTE_MOCK_NOT_FOUND;
			String[] params = new String[] { MessageFormat.format(MOCK_FILENAME_TEMPLATE, key), key };
			throw new VopRuntimeException(msgkey, MessageSeverity.WARN, HttpStatus.OK, e, params);
		}
		return resource;
	}
